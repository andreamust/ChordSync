...
class ForcedAlignmentLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # wrong_probabilities_loss = get_wrong_probabilities(predictions, targets)
        # calculate loss
        loss = F.cross_entropy(predictions, targets)

        # loss += wrong_probabilities_loss
        return loss
...
class DeepChromaConformer(L.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.fc_dimensionality_reduction = nn.Linear(self.conformer_dimension, 36)
        # self.fc_dimensionality_augmentation = nn.Linear(36, self.conformer_dimension)

        # conformer
        self.conformer_decoder = Conformer(
            input_dim=self.conformer_dimension,
            num_heads=4,
            num_layers=4,
            ffn_dim=128,
            dropout=0.4,
    ...
    def forward(self, x):
        ...
        # # fully connected
        # fc_pre_conformer = self.fully_connected1(convolution)
        # fc_dimensionality_reduction = self.fc_dimensionality_reduction(
        #     conformer_encoder
        # )
        # fc_dimensionality_reduction = torch.relu(fc_dimensionality_reduction)
        # fc_dimensionality_augmentation = self.fc_dimensionality_augmentation(
        #     fc_dimensionality_reduction
        # )
        # fc_dimensionality_augmentation = torch.relu(fc_dimensionality_augmentation)

        # # conformer
        # conformer_decoder, _ = self.conformer_decoder(
        #     fc_dimensionality_augmentation, durations
        # )
    ...
    def _shared_step(self, batch, batch_idx, mireval: bool = True):
        ...
        # convert simplified to one-hot
        # simplified_one_hot = create_change_tensor(simplified).float()
        # simplified_one_hot_smooth = smooth_changes_gaussian(
        #     simplified_one_hot, kernel_size=11, sigma=1.7
        # )
        ...
        # forward pass
        out_majmin = self(x)
        # out_majmin = smooth_probabilities(out_majmin, majmin_sequence)
...
def main(
    ...
    model = DeepChromaConformer(
        num_classes_root=vocabularies["root"] + 1,
        num_classes_mode=vocabularies["mode"] + 1,
        num_classes_chord=vocabularies["simplified"] + 1,
        num_classes_majmin=vocabularies["majmin"] + 1,
